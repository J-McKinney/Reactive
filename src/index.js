...
class App {
  constructor(container) {
    ...
    this.config = {
      backgroundColor: new Color("#282c34"),
      // backgroundColor: new Color("#fbc40d"),
      // backgroundColor: new Color("#0d021f"),
      cameraSpeed: 0, // 0
      cameraRadius: 4.5, // 4.5
      particlesSpeed: 0, // 0
      particlesCount: 3000, // 3000
      bloomStrength: 1.45, // 1.45
      bloomThreshold: 0.34, // 0.34
      bloomRadius: 0.5, // 0.5
    };
  ...
  init() {
    ...
    audioBtn.addEventListener(
      ...
      () => {
        ...
        tl.set("#audio-btn-icon", { transition: "none" })
          .to("#audio-btn-label", { "--clip": 1, duration: 0.6 })
          .to(
            "#audio-btn-icon",
            { rotation: 140, scale: 0.8, duration: 0.5 },
            "<0.05"
          )
          .to("#audio-btn-icon", { opacity: 0, duration: 0.65 }, "<")
          .to(
            audioBtn,
            { width: 66, ease: "back.inOut(2)", duration: 0.9 },
            "<0.2"
          );
      },
      { once: true }
  ...
  _update() {
    ...
    this.particles.material.uniforms.uTime.value +=
      0.05 * this.config.particlesSpeed;
    ...
    if (!!this.analyser) {
      ...
      this.camera.position.x =
        Math.sin(this.tick * 0.63) * 2.7 * this.config.cameraSpeed;
      this.camera.position.y =
        Math.sin(this.tick * 0.84) * 2.15 * this.config.cameraSpeed;
      this.camera.position.z =
        Math.cos(this.tick * 0.39) *
        this.config.cameraRadius *
        this.config.cameraSpeed;
  ...
  _createParticles() {
    ...
    this.particles = new InstancedUniformsMesh(
      geom,
      material,
